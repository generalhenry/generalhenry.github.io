/*global fc: true, soundManager: true */
function FireworksController() {
  ...
  self.getWindowCoords = function() {
    ...
    self.scrollY = ~~(window.scrollY||document.documentElement.scrollTop||document.body.scrollTop);
...
function Firework(oC,startX,startY,burstX,burstY,burstType,nRadius,nParticles,nCircles,allowRandom,obeyBoundaries) {
  ...
  this.createBurst = function(circles,nMax,rMax,type) {
    ...
    var circleTypes = [type,circles>1?~~(Math.random()*fc.particleTypes):type];
    var thisType = null;

    for (i=0; i<circles; i++) {
      particlesPerCircle[i] = ~~(nMax*(i+1)/circles*1/circles)||1; // hack - nMax*(i+1)/circles;
...
function createFirework(nRadius,nParticles,nCircles,nBurstType,startX,startY,burstX,burstY,allowRandom,obeyBoundaries) {
  ...
  if (fc.isEmpty(startX)) {
    startX = ~~(Math.random()*fc.canvasX);
  } else {
    startX = ~~(fc.canvasX*startX/100);
  }

  if (fc.isEmpty(startY)) {
    startY = fc.canvasY-fc.particleXY;
  } else {
    startY = fc.canvasY-fc.screenY+~~(fc.screenY*startY/100);
  }

  if (fc.isEmpty(burstX)) {
    burstX = ~~(fc.canvasX*0.1+(Math.random()*fc.canvasX*0.8));
  } else {
    burstX = ~~(fc.canvasX*burstX/100);
  }

  if (fc.isEmpty(burstY)) {
    burstY = fc.canvasY-~~(Math.random()*fc.screenY);
  } else {
    burstY = fc.canvasY-~~(fc.screenY*(100-burstY)/100);
  }

  if (fc.isEmpty(nBurstType)) {
    nBurstType = ~~(Math.random()*fc.particleTypes);
  }

  if (fc.isEmpty(nRadius)) {
    nRadius = 64+~~(Math.random()*fc.screenY*0.75);
  } else if (nRadius.toString().indexOf('%')>=0) {
    nRadius = ~~(~~(nRadius)/100*fc.screenY);
  } else if (nRadius.toString().indexOf('.')>=0) {
    nRadius = ~~(nRadius*fc.screenY);
  } else {
    nRadius = ~~(nRadius*fc.screenY/100);
  }

  if (fc.isEmpty(nParticles)) {
    nParticles = 4+~~(Math.random()*64);
  }
